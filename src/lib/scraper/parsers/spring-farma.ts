...
/**
 * Parser pentru Spring Farma (springfarma.com)
 */
export const springFarmaParser: RetailerParser = {
  name: 'Spring Farma',
  ...
  parse(html: string, url: string): ParseResult {
    try {
      ...
      // Verificăm selectori specifici
      if ($('.out-of-stock, .unavailable, .product-unavailable').length > 0) {
        isInStock = false
      }

      // Prețul - Spring Farma (Magento)
